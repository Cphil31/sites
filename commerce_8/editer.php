...
if (filter_input(INPUT_POST, 'submit')) {
    ...
    if (!$tabErreur) {
        ...
        $req = "INSERT INTO produit VALUES ({$produit->id_produit}, {$produit->id_categorie}, {$pdo->quote($produit->nom)}, {$pdo->quote($produit->ref)}, {$produit->prix}) ON DUPLICATE KEY

         UPDATE produit  id_categorie = {$produit->id_categorie}, nom = {$pdo->quote($produit->nom)}, ref = {$pdo->quote($produit->ref)}, prix = {$produit->prix}";
        $pdo->exec($req);
